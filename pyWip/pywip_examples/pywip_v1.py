...
def arrow(xcol,ycol,taper=45,vent=0.0,fill='s',color=None,size=None,style=None,
   width=None,limits=None,logx=None,logy=None):
   '''Draw a single arrow.
   ...
   if not _isseq(xcol):
      _error('xcol keyword for arrow command must be a list/tuple!')
   if not _isseq(ycol):
      _error('ycol keyword for arrow command must be a list/tuple!')
   if len(xcol) != 2:
      _error('xcol keyword for arrow command must have two elements!')
   if len(ycol) != 2:
      _error('ycol keyword for arrow command must have two elements!')
...
def axis(xaxis=None,yaxis=None,color=None,font=None,size=None,style=None,
   ...
   if not yaxis:
      ...
      if nx > 0 or _curpanel%abs(nx) == 0: # space between panels
         ylab = ylab + 'n'                 # or on first column 
...
def contour(image,header,levels,routine='smooth',limits=None,color=None,
   ...
   r'''Draw contours with the specified levels.
      ...
      border  - Set to true if we want to draw the border instead of the
                contour levels'''
   fp = _wipopen()
   ...
   if borderFlag:
      ...
      if style == '--': # must set to fast to get dashed contours to show up
         routine = 'fast'
      if not style: # check to see if default line style is --
         if _lstyle == _translatelstyle('--'):
            routine = 'fast'
      if routine == 'smooth':
         fp.write('contour t\n')
      elif routine == 'fast':
...
def plot(xcol,ycol,datafile=None,color=None,size=None,style='o',width=None,
   ...
   '''Plot data from a file or input lists/tuples.
      ...
      fillcolor - Color to fill symbols with.  Only available for five-point
                  ...
                  You can specify an integer, which will then read the specified
                  color from the datafile for each point.
   ...
   fp = _wipopen()
   pan = _updatepanel(limits,logx,logy)
   plotcurve = curve(color,size,style,width,text,fillcolor) #stuff for a legend
   if not datafile:
      if _isseq(xcol) and _isseq(ycol):
         num = len(xcol)
         if num > 10:
...
def poly(xcol,ycol,datafile=None,color=None,style=None,width=None,fill='h',
   fillcolor=None,limits=None,logx=False,logy=False):
   '''Draw a closed polygon.
      ...
      fill      - Fill style for polygon, defaults to hollow (no fill)
   ...
   if not datafile:
      ...
      xcol = 1
      ycol = 2
   if fill != 'h' and not fillcolor:
      _error('If fill style is not hollow, you must specify a fillcolor!')
   ...
   if pan['logy']:
      fp.write('log y\n')
   _writeoptions(fp,None,None,None,style,width)
   if fillcolor:
      _writeoptions(fp,fillcolor,None,None,None,None)
      fillstyle = _translatefill(fill)
      fp.write('fill %s\n' %fillstyle)
      fp.write('poly\n')
      _resetdefaults(fp,fillcolor,None,None,None,None)
   _writeoptions(fp,color,None,None,None,None)
   fp.write('fill %s\n' %_translatefill('h'))
   fp.write('poly\n')
   _resetdefaults(fp,color,None,None,style,width)
...
def rect(xmin,xmax,ymin,ymax,color=None,style=None,width=None,fill='h',
   fillcolor=None,limits=None,logx=False,logy=False):
   ...
   poly([xmin,xmax,xmax,xmin],[ymin,ymin,ymax,ymax],color=color,style=style,
      width=width,fill=fill,fillcolor=fillcolor,limits=limits,logx=logx,
      logy=logy)
...
def vector(xcol,ycol,anglecol,lengthcol,datafile=None,taper=45,vent=0.0,
   color=None,size=None,width=None,fill='s',limits=None,logx=None,logy=None):
   '''Draw a vector field.

      lengthcol is a column with the length of each arrow and anglecol is the 
      angle for each arrow, in degrees.  Zero degrees is west, or negative x
      direction, on a coordinate axes, and increase clockwise.
...
def wedge(side,offset,thickness,minmax=None,boxarg=None,color=None,font=None,
   ...
   '''Draw a halftone wedge on the image.
      ...
      boxarg    - argument for box style.  Uses the same lettering as done by
                  the axis function.  Defaults to a box with numbers.
...
def _maketempfile2(xcol,ycol,datafile,xerr=None,yerr=None):
   ...
   if not os.path.exists(datafile):
      _error('Cannot find file %s for reading!' %datafile)
   fp1 = open(datafile,'r')
   fp2 = open(blah,'w')
   line = fp1.readline()
   numwritten = 0
...
def _updatepanel(limits,logx,logy,image=None,overlap=None):
   ...
   if limits:
      if _isseq(limits):
         ...
         _error('Unknown value %s for the limits keyword!' %str(limits))
   else:
...
def _writelimits(fp):
   ...
   if not limits:
      ...
      if _isseq(limits):
         ...
         if logx:
            if limits[0] == 0:
               _warning('Minimum x value is 0, setting to 1e-5.')
               minx = 1e-5
            else:
               minx = limits[0]
            if limits[1] == 0:
               _warning('Maximum x value is 0, setting to 1e-5.')
               maxx = 1e-5
            else:
               maxx = limits[1]
            fp.write('%f %f ' %(math.log10(minx),math.log10(maxx)))
         else:
            fp.write('%f %f ' %(limits[0],limits[1]))
         if logy:
            if limits[2] == 0:
               _warning('Minimum y value is 0, setting to 1e-5.')
               miny = 1e-5
            else:
               miny = limits[2]
            if limits[1] == 0:
               _warning('Maximum y value is 0, setting to 1e-5.')
               maxy = 1e-5
            else:
               maxy = limits[3]
            fp.write('%f %f ' %(math.log10(miny),math.log10(maxy)))
         else:
